...
echo "<table class=\"table table-striped table-bordered bootstrap-datatable datatable responsive\">
   ...
   $query  =mysql_query("SELECT siswa.nm_siswa,peminjaman.no_pinjam,peminjaman.status,peminjaman.tgl_pinjam,peminjaman.tgl_kembali FROM peminjaman, siswa WHERE peminjaman.nisn=siswa.nisn AND  status='Pinjam' ORDER BY no_pinjam",$koneksidb);
   //$tampil = mysql_query($query,$koneksidb);
    $no = 1;
    while ($tyo = mysql_fetch_array($query)):
         $Kode = $tyo['no_pinjam'];
         echo "<tr><td>$no</td>
             <td>$tyo[no_pinjam]</td>
